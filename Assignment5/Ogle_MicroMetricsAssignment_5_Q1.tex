...
very opposed or very for autonomy. Additionally, RD necessitates that in
...
cutoff. Clark needs to estimate around the cutoff, 50, which is inside
the (15,85) interval. Other columns have functions of vote share because
vote share is the forcing variable in this Fuzzy Regression
Discontinuity model. Fuzzy RD exploits discontinuities in the
probability of treatment conditional on a variate. The discontinuity,
here is Vote share, and it becomes the IV for the treatment status.
Which is why Table 3a includes functions of vote share both on their own
and interacted with the win/lose variable.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{model_d_}\DecValTok{1}\NormalTok{ <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(vote }\OperatorTok{~}\StringTok{ }\NormalTok{win }\OperatorTok{+}\StringTok{ }\NormalTok{vote }\OperatorTok{+}\StringTok{ }\NormalTok{vote}\OperatorTok{*}\NormalTok{lose }\OperatorTok{+}\StringTok{ }\NormalTok{vote}\OperatorTok{*}\NormalTok{win }\OperatorTok{+}\StringTok{ }\NormalTok{(vote}\OperatorTok{^}\DecValTok{2}\NormalTok{)}\OperatorTok{*}\NormalTok{lose }\OperatorTok{+}\StringTok{ }\NormalTok{(vote}\OperatorTok{^}\DecValTok{2}\NormalTok{)}\OperatorTok{*}\NormalTok{win }\OperatorTok{+}\StringTok{ }\NormalTok{percentage_change, }\DataTypeTok{data =}\NormalTok{ restrict)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{stargazer}\NormalTok{(model_d_}\DecValTok{1}\NormalTok{, }\DataTypeTok{column.labels=}\KeywordTok{c}\NormalTok{(}\StringTok{"Non-Grammar Schools with Vote Shares in [15,85] interval"}\NormalTok{), }\DataTypeTok{title=}\StringTok{"Regression Results (d)"}\NormalTok{, }\DataTypeTok{align=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{notes =} \StringTok{"Standard errors in parentheses"}\NormalTok{, }\DataTypeTok{notes.align =} \StringTok{"l"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## % Date and time: Sun, Feb 20, 2022 - 18:19:54
